...
We'll compare LangChain to Substrate using a simple example chaining two LLMs. In this example, Substrate requires **60% less code**, and runs faster.
...
Substrate requires less code, and also runs faster – because Substrate is a workflow execution and inference engine. The same LLM chain runs about 2x faster on Substrate (in this particular example, using Mistral 7B on Together).
